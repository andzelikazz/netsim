...
void generate_structure_report(const Factory &f, std::ostream &os) {
  ...
  for (auto it = f.ramp_cbegin(); it != f.ramp_cend(); it++) {
    ...
    for (auto iterator = it->receiver_preferences_.cbegin();
      ...
      } else if (iterator->first->get_receiver_type() ==
                 ReceiverType::STOREHOUSE) {
...
void generate_simulation_turn_report(const Factory &f, std::ostream &os,
  ...
  for (auto it : workers) {
    ...
    if (iterator->get_processing_buffer().has_value()) {
      os << "  PBuffer: #" << iterator->get_processing_buffer()->get_id()
         << " (pt = "
         << std::to_string(t - iterator->get_package_processing_start_time() +
                           1)
         << ")\n";
    } else {
      os << "  PBuffer: (empty)\n";
    ...
    if (iterator->get_queue()->empty()) {
      ...
      os << "  Queue:";
      for (auto i = iterator->get_queue()->cbegin();
           i != iterator->get_queue()->cend(); i++) {
