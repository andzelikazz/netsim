...
void perform_report_check(
    std::function<void(std::ostringstream &)> &reporting_function,
  ...
  for (std::size_t i = 0; i < output_lines.size(); ++i) {
    ASSERT_EQ(output_lines[i], expected_report_lines[i])
        << "(in element no.: " << (i + 1) << ")";
  }
}

void perform_turn_report_check(
    const Factory &factory, Time t,
    std::vector<std::string> &expected_report_lines) {
  std::function<void(std::ostringstream &)> reporting_function =
      [&factory, t](std::ostringstream &oss) {
        generate_simulation_turn_report(factory, oss, t);
      };
  perform_report_check(reporting_function, expected_report_lines);
}

void perform_structure_report_check(
    const Factory &factory, std::vector<std::string> &expected_report_lines) {
  std::function<void(std::ostringstream &)> reporting_function =
      [&factory](std::ostringstream &oss) {
...
TEST(ReportsTest, StructureReport_R2W2S2) {
  ...
  factory.add_worker(
      Worker(1, 1, std::make_unique<PackageQueue>(PackageQueueType::FIFO)));
  factory.add_worker(
      Worker(2, 2, std::make_unique<PackageQueue>(PackageQueueType::LIFO)));
  factory.add_storehouse(Storehouse(1));
...
TEST(ReportsTest, TurnReportPackageInProcessingBuffer) {
  ...
  factory.add_ramp(Ramp(1, 10));
  factory.add_worker(
      Worker(1, 2, std::make_unique<PackageQueue>(PackageQueueType::FIFO)));
  factory.add_storehouse(Storehouse(1));
